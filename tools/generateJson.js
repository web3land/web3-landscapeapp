...
async function main() {
  ...
  _.each(itemsWithExtraFields, function (item) {
    const membership = (function () {
      ...
      var parentWithMembership = null
      if (item.crunchbaseData) {
        parentWithMembership = _.find(item.crunchbaseData.parents, function (parent) {
  ...
  */
  ...
  const lookups = {
    ...
    license: pack([]),//pack(generateLicenses()),
    headquarters: pack([]),//pack(generateHeadquarters()),
    crunchbaseSlugs: [],//generateCrunchbaseSlugs(),
    languages: [],//generateLanguages(),
    companyTypes: pack([]),//pack(generateCompanyTypes()),
    industries: pack([]),//pack(generateIndustries())
  }
